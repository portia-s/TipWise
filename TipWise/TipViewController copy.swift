...
class TipViewController: UIViewController, UITextFieldDelegate, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        let myCustomTipPercent = Int(userSettingR.0 * 100)
        tipValueControl.setTitle("\(myCustomTipPercent)%", forSegmentAt: 3)
        
        calculateTip(self)
/*
        let tipPercent = [0.10, 0.15, 0.20, Double(userSettingR.0)]
        ...
        let formattedMoney = updateMoneyLabels(tipMoney: tipAmount, totalMoney: totalAmount, splitMoney: splitLabelValue, decimalEnable: userSettingR.1)
        tipLabel.text = formattedMoney.0
        totalLabel.text = formattedMoney.1
        splitLabel.text = formattedMoney.2
*/
...
//                }
    ...
    @IBAction func calculateTip(_ sender: AnyObject) {
        ...
        //make certain billAmount is valid double, in case TF is nil then default is 0
        let billAmount = Double(amountTextField.text!) ?? 0.00
        let tipAmount = billAmount * tipPercent[tipValueControl.selectedSegmentIndex]
        let totalAmount = billAmount + tipAmount
        totalBillWithTip = totalAmount
...
//        let formattedMoney = updateMoneyLabels(tipMoney: tipAmount, totalMoney: totalAmount, splitMoney: splitLabelValue, decimalEnable: readFromNSUD().1)
//        tipLabel.text = formattedMoney.0
//        totalLabel.text = formattedMoney.1
//        splitLabel.text = formattedMoney.2
        if readFromNSUD().1 == true {
...
/*
    func updateMoneyLabels(tipMoney: Double, totalMoney: Double, splitMoney: Double, decimalEnable: Bool) -> (String, String, String) {
        //var moneyFormatter = NumberFormatter()
        //moneyFormatter.numberStyle = NumberFormatter.Style.currency
        
        if decimalEnable == true {
            //tipLabel.text = String(format: "$%0.2f", tipAmount)
            let tip = myFormatter.string(from: tipMoney as NSNumber)
            //totalLabel.text = String(format: "$%0.2f", totalAmount)
            let total = myFormatter.string(from: totalMoney as NSNumber)
            //splitLabel.text = String(format: "$%0.2f", splitLabelValue)
            let split = myFormatter.string(from: splitMoney as NSNumber)
